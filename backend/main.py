...
def _build_damage_lists(type_names: list[str], type_cache: dict):
    ...
    def categorize(effectiveness: dict[str, float]) -> list[str]:
        return [name for name, multiplier in sorted(effectiveness.items()) if multiplier in {0.0, 0.5, 2.0}]
    ...
    no_damage_to = categorize(outgoing_effectiveness)
    half_damage_to = [name for name, multiplier in sorted(outgoing_effectiveness.items()) if multiplier == 0.5]
    double_damage_to = [name for name, multiplier in sorted(outgoing_effectiveness.items()) if multiplier == 2.0]
    no_damage_from = categorize(incoming_effectiveness)
    half_damage_from = [name for name, multiplier in sorted(incoming_effectiveness.items()) if multiplier == 0.5]
    double_damage_from = [name for name, multiplier in sorted(incoming_effectiveness.items()) if multiplier == 2.0]
...
def seed_db(force: bool = False):
    ...
    try:
        ...
        # Der API-Response kann auch None sein, wenn der Request fehlschlägt.
        # Dann ist die weitere Verarbeitung nicht sinnvoll und wir brechen sauber ab.
